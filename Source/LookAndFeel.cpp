...
void RotaryKnobLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, [[maybe_unused]] int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, juce::Slider& slider)
{
	...
	auto lineWidth = 3.0f;
	auto arcRedius = radius - (lineWidth / 2.0f);

	juce::Path backgroundArc;
	backgroundArc.addCentredArc(center.x, center.y, arcRedius, arcRedius, 0.0f, rotaryStartAngle, rotaryEndAngle, true);
	...
	juce::Point<float> dialEnd(center.x + dialRadius * std::sin(toAngle), center.y - dialRadius * std::cosf(toAngle));
	...
	if (slider.isEnabled()) {
		...
		juce::Path valueArc;
		valueArc.addCentredArc(center.x, center.y, arcRedius, arcRedius, 0.0f, fromAngle, toAngle, true);
